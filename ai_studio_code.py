...
with tab2:
    ...
    with st.expander("📜 交易劇本：【MSCI 波動戰法】 (點擊展開)", expanded=True):
        ...
        2.  **生效日 (月底)：**
            ...
            *   **操作：** 若你持有納入股，不要在盤中賣，**掛尾盤市價賣**，通常能賣在當天最高點 (因為外資被迫要買)。

        #### ❌ 避雷針 (右欄剔除股)
        *   若股票在右欄「潛在剔除區」，且外資連續賣超，**不要去接刀**。
        *   MSCI 剔除的股票，往往會經歷長達數月的修正 (因為外資部位太大，賣不完)。
        """)
...
with tab3:
    ...
    with st.expander("📜 交易劇本：【主力潛伏跟單術】 (點擊展開)", expanded=True):
        ...
        *   **確認：** 看到符合上述特徵的股票，檢查新聞是否有「高殖利率」題材。
        *   **進場：** 在公告前 1 個月佈局。
        *   **出場：** 公告名單當天大漲時，賣給看新聞追價的散戶。
        """)
